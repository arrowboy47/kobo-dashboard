...
-- ==== CONFIG (edit these) ====
local URL = "http://192.168.7.172:3333/today.png" -- URL to fetch the PNG from. If using a local server, use the device's LAN IP. If using Docker, you might not need to change anything
...
function Agenda:onDispatcherRegisterActions()
	Dispatcher:registerAction(
		"agenda_start",
		{ category = "none", event = "AgendaStart", title = _("Agenda Display: Start"), general = true }
	)
	Dispatcher:registerAction(
		"agenda_stop",
		{ category = "none", event = "AgendaStop", title = _("Agenda Display: Stop"), general = true }
	)
end
...
function Agenda:open()
	...
	self._sched_cb = self._sched_cb or function()
		self:_tick()
	end
	self:_tick()
end
...
function Agenda:close()
	if self._sched_cb then
		UIManager:unschedule(self._sched_cb)
	end
	if self._widget then
		UIManager:close(self._widget, "full")
		self._widget = nil
